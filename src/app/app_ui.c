...
static LRESULT CALLBACK settings_window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_CREATE:
        ...
        g_app.startup_checkbox = CreateWindowExW(0, L"BUTTON", L"登录时自动启动",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_OWNERDRAW,
            16, 14, 300, 24, window, (HMENU)(INT_PTR)IDC_STARTUP_CHECKBOX, g_app.app_instance, NULL);
        label = CreateWindowExW(0, L"STATIC", L"Fn 键模式：", WS_CHILD | WS_VISIBLE,
            16, 48, 100, 20, window, NULL, g_app.app_instance, NULL);
        g_app.fn_lock_radio = CreateWindowExW(0, L"BUTTON", L"锁定 Fn（F1–F12 优先）",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_GROUP | BS_OWNERDRAW,
            26, 68, 280, 22, window, (HMENU)(INT_PTR)IDC_FN_LOCK_RADIO, g_app.app_instance, NULL);
        g_app.fn_unlock_radio = CreateWindowExW(0, L"BUTTON", L"解除锁定（媒体键优先）",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_OWNERDRAW,
            26, 92, 280, 22, window, (HMENU)(INT_PTR)IDC_FN_UNLOCK_RADIO, g_app.app_instance, NULL);
        g_app.tray_checkbox = CreateWindowExW(0, L"BUTTON", L"显示任务栏图标",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_OWNERDRAW,
            16, 124, 300, 24, window, (HMENU)(INT_PTR)IDC_TRAY_CHECKBOX, g_app.app_instance, NULL);
        g_app.status_label = CreateWindowExW(0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_LEFT,
            16, 154, 325, 20, window, NULL, g_app.app_instance, NULL);
        close_button = CreateWindowExW(0, L"BUTTON", L"退出程序", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_OWNERDRAW,
            128, 181, 104, 28, window, (HMENU)(INT_PTR)IDC_CLOSE_BUTTON, g_app.app_instance, NULL);
