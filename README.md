...
为实现自动恢复，`k380FnAutoLock.exe` 需要在后台持续运行。它使用 Windows 自带的系统托盘和原生控件，不依赖额外 GUI 框架。右键托盘图标可打开菜单；左键图标或选择“打开设置”会显示一个小型设置窗口。设置窗口右上角的 X 只关闭窗口；窗口底部的红色“退出程序”按钮和托盘菜单中的同名选项会结束当前进程，但保留登录自启设置。托盘图标由程序使用 Windows GDI 绘制，无需单独的图标文件或额外依赖；若只想手动设置一次，可以仅使用两个手动命令。
